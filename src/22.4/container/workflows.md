...
tunnel the session through TLS (_‘off’_).
  - `MTA_TLS_CERTCHECK`: Enable or disable checks of the server certificate. They are enabled by default. (WARNING: When the checks are disabled, TLS sessions will not be secure!)
